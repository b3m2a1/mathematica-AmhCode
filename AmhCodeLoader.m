...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated AmhCode loader file *)


BeginPackage["AmhCode`"];
...
$PackageContexts={
		"AmhCode`",
		"AmhCode`Private`Package`",
		"AmhCode`Private`Hidden`"
		};
$PackageDeclared=
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,AmhCode],"`"]
		];
...
PackageScopeBlock[e_,scope_String:"Hidden"]:=
	With[{newcont="AmhCode`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="AmhCode`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@AmhCode`Private`Package`$loadAbort;!#)&@AmhCode`Private`Package`$loadAbort,
	AmhCode`Private`Package`PackagePostProcessContextPathReassign[]
	]