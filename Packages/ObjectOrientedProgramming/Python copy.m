...
AncestorClasses::usage="Set classes to inherit from"
...
Protect@MessageName;
(*Overloads the necessary operators to get MathematicaClass[_Symbol] to operate with :: like most languages do with .*)
ClassOverload:=(
(*Instantiation*)
MathematicaClass/:HoldPattern[MathematicaClass[v_?ClassSymbolQ][args___]]:=
	With[{o=v[args]},
...
(*Call*)
MathematicaClass/:HoldPattern[MathematicaClass[v_?InstanceSymbolQ][args___]]:=
	With[{o=v[args]},
		If[o===Unevaluated[v[args]],
...
repr[m:MathematicaClass[_Symbol?ObjectSymbolQ],format_]:=
Block[{
	BoxForm`sufficientVersionQ,
	EntityFramework`Private`GetEntityName=(#[[2]])&,
	EntityFramework`Private`OKEntityNameQ=(True&)
	},
	BoxForm`sufficientVersionQ[_]:=True;
...
MakeBoxes[MathematicaClassBoxForm[m:MathematicaClass[_Symbol?ObjectSymbolQ]],StandardForm]:=repr[m,StandardForm];
MakeBoxes[MathematicaClassBoxForm[m:MathematicaClass[_Symbol?ObjectSymbolQ]],TraditionalForm]:=repr[m,StandardForm];
Format[m:MathematicaClass[sym_Symbol]/;($AutoRepresentation&&ObjectSymbolQ@sym)]:=
...
(* ::Text:: *)
(*NEED TO REWRITE THIS SO THAT EACH OBJECT LOOKS LIKE funcType[func,attrDict] to make look up constant time and whatnot*)
(**)
(*I think the crucial inefficiencies in the class system arise here.*)


(* ::Subsubsection::Closed:: *)
(*Making Function Objects*)


(*With[{metPat=StringRulePattern["PassCls","SuperPassCls","BoundTo"]},
	ClassMethod[f_,attrDict:Association[metPat...]:<||>,attrs:metPat..]:=ClassMethod[f,Merge[{attrDict,<|attrs|>},First]]
];
With[{metPat=StringRulePattern["PassCls","PassObj","SuperPassCls","SuperPassObj","BoundTo"]},
	BoundMethod[f_,attrDict:Association[metPat...]:<||>,attrs:metPat..]:=BoundMethod[f,Merge[{attrDict,<|attrs|>},First]]
];
With[{metPat=StringRulePattern["PassCls","PassObj","SuperPassCls","SuperPassObj","BoundTo"]},
	BoundProperty[f_,attrDict:Association[metPat...]:<||>,attrs:metPat..]:=BoundProperty[f,Merge[{attrDict,<|attrs|>},First]]
];*)
...
(*FunctionGetAttribute[(_)[_,attrDict_Association],attr_String]:=attrDict[attr];*)
...
(*
FunctionReplaceAttribute[function_,attr_String,obj_:None,head_:Rule]:=Replace[function,(h_)[f_,d_]\[RuleDelayed]h[f,Append[d,head[attr,obj]]]];
FunctionReplaceAttribute[function_,attr:(_Rule|_RuleDelayed)]:=Replace[function,(h_)[f_,d_]\[RuleDelayed]h[f,Append[d,attr]]];
FunctionReplaceAttribute[function_,attrs:(_Rule|_RuleDelayed)..]:=Replace[function,(h_)[f_,d_]\[RuleDelayed]h[f,Merge[{d,<|attrs|>},Last]]];
*)
...
(*
This is the upcoming version, once the second element of each functional type becomes an association

CallFunction[ClassMethod[function_,attrDict_Association],args___]:=
With[{hasCls=KeyMemberQ[attrDict,"PassCls"]},
	If[Not@hasCls&&{args}==={},
		Message[ClassMethod::nocls,function];
		$Failed,
		With[{nargs=Sequence@@If[Not@hasCls,{args}[[2;;]],{args}],
				cls=Replace[attrDict["PassCls"],_Missing:>{args}[[1]]]},
			With[{supercls=Replace[attrDict["SuperPassCls"],_Missing->cls]},
...
CallFunction[BoundMethod[function_,attrDict_Association],args___]:=
With[{hasObj=KeyMemberQ[attrDict,"PassObj"]},
	If[Not@hasObj&&{args}==={},
		Message[BoundMethod::noobj,function];
		$Failed,
		With[{nargs=Sequence@@If[Not@hasObj,{args}[[2;;]],{args}],
			obj=Replace[attrDict["PassObj"],_Missing\[RuleDelayed]{args}[[1]]]},
			With[{cls=Replace[attrDict["PassCls"],_Missing\[RuleDelayed]ObjectType@obj]},
				With[{
					supercls=Replace[attrDict["SuperPassCls"],_Missing\[Rule]cls],
					superobj=Replace[attrDict["SuperPassObj"],_Missing\[Rule]obj]},
					Block[{$SuperclassConstructionPair={supercls,superobj}},
						function[obj,nargs]]
					]
				]
			]
...
parentClassesKey=MathematicaClass$AncestorClasses;
...
makeinstance[class_Symbol,
	...
	With[{initargs=Sequence@@argconfig[[1]],initops=Sequence@@argconfig[[2]],
		initFunction=BoundMethod[init,"PassObj"->MathematicaClass@self,"PassCls"->MathematicaClass@class]},
		$InstanceSymbols[self]=self;
		...
		self[parentClassesKey]=AncestorClasses@MathematicaClass@class;
		(*-------------------------------------------------------------------------------*)
		...
		(*-------------------------------------------------------------------------------*)
		self["$GetAttribute",attr_]:=
			With[{getattrib=Replace[getboundmethod[self,getattr[self,"$GetAttribute"]],
					{{True,v_}:>v,{False,v_}:>(setattr[self,"$GetAttribute",v];v)}]},
		...
		self["$SetAttribute",attr_String,value_,mode_]:=self["$GetAttribute","$SetAttribute"][attr,value,mode];
		self["$SetAttribute",attr_,value_,mode_]:=
			With[{s=StringSplit[ToString[Unevaluated[attr]],{"`"}][[-1]]},
				self["$SetAttribute",s,value,mode]
				];
		self["$DeleteAttribute",attr_String]:=self["$GetAttribute","$DeleteAttribute"][attr];
		self["$DeleteAttribute",attr_]:=
			With[{s=StringSplit[ToString[Unevaluated[attr]],{"`"}][[-1]]},
				self["$DeleteAttribute",s]
				];
...
With[{
	class=Replace[classname,_String:>Symbol[classname<>"$"<>ToString@$ClassCounter]],
	...
	class[AttributeTable]=Merge[{
		...
		Association@@Table[
			...
			|>},First];
	class[InheritFrom]=With[{parent=OptionValue[AncestorClasses]},Flatten@{parent,(AncestorClasses/@parent)}];
	class[SearchPath]=Prepend[class[InheritFrom],class];
	(*-------------------------------------------------------------------------------*)
	class["$GetAttribute",attr_String]:=getattr[class,"$GetAttribute"][attr];
	class["$GetAttribute",attr:Except[_String]]:=
		With[{s=StringSplit[ToString[Unevaluated[attr]],{"`"}][[-1]]},
				class["$GetAttribute",s]
			];
	class["$SetAttribute",attr_String,value_,mode_:"normal"]:=class["$GetAttribute","$SetAttribute"][attr,value,mode];
	class["$SetAttribute",attr:Except[_String],value_,mode_:"normal"]:=
		With[{s=StringSplit[ToString[Unevaluated[attr]],{"`"}][[-1]]},
			class["$SetAttribute",s,value,mode]
			];
	class["$DeleteAttribute",attr_String]:=class["$GetAttribute","$DeleteAttribute"][attr];
	class["$DeleteAttribute",attr:Except[_String]]:=
		With[{s=StringSplit[ToString[Unevaluated[attr]],{"`"}][[-1]]},
			class["$DeleteAttribute",s]
			];
	(*-------------------------------------------------------------------------------*)
	SetAttributes[class,HoldRest];
	$ClassCounter++;
	$ClassSymbols[class]=class;
...
Do[With[{c=ToExpression@p},
	...
	(*c/:HoldPattern[ConstructClass[a___,c[args__],b___]]:=(
		...
		PostLoadFunction}}]
AncestorClasses/:HoldPattern[Set[AncestorClasses,v_]]:=AncestorClasses@v;
AncestorClasses/:HoldPattern[SetDelayed[AncestorClasses,v_]]:=AncestorClasses@v;
...
ConstructClass[name_,properties___]:=Block[{
	...
	builtClass=MathematicaClassConstructor[name,
		AncestorClasses->parentClassList,
...
*)
ClassCount:=Length@Keys@$ClassSymbols;
InstanceCount:=Length@Keys@$InstanceSymbols;
ClearClasses:=Do[GetAttribute[MathematicaClass@x,"$Del"][x],{x,Keys@$ClassSymbols}];
ClearInstances:=Do[GetAttribute[MathematicaClass@x,"$Del"],{x,Keys@$InstanceSymbols}];
...
ClassSymbolQ[sym_]:=KeyMemberQ[$ClassSymbols,sym];
InstanceSymbolQ[sym_]:=KeyMemberQ[$InstanceSymbols,sym];
ObjectSymbolQ[sym_]:=ClassSymbolQ@sym||InstanceSymbolQ@sym;
$ObjectSymbols:=Merge[{$ClassSymbols,$InstanceSymbols},First];
ClassQ[obj_]:=Replace[obj,{MathematicaClass[s_Symbol]:>ObjectSymbolQ@s,_:>False}];
InstanceQ[obj_]:=Replace[obj,{MathematicaClass[s_Symbol]:>InstanceQ@s,_:>False}];


(* ::Subsubsection::Closed:: *)
(*"AncestorClasses", ObjectType*)


AncestorClasses[m:MathematicaClass[_Symbol]]:=m[parentClassesKey];
(*With[{o=ObjectType[m]},
...
With[{classSet={classes}/.MathematicaClass[s_]:>s},
	Replace[TrueQ@MemberQ[classSet,ObjectType[obj]],
		False:>With[{tests=If[ClassQ@obj,AncestorClasses@obj/.MathematicaClass[s_]:>s,{}]},
...
ObjectString[obj_]:=If[ClassQ@obj,
						...
						ToString@obj];
ObjectRepresentation[obj_]:=If[ClassQ@obj,
...
HasAttribute[obj_,attr_]:=
	If[ClassQ@obj,
		Replace[Do[If[KeyMemberQ[c[attributesKey],attr],Return[True]],{c,Prepend[AncestorClasses[obj],obj]}],
			Except[True]:>False
			],
...
With[{parentclasses=AncestorClasses[MathematicaClass[class]]},
	self::obj=passobj;
	self::cls=class;
...
SaveObject[MathematicaClass[sym_?ClassSymbolQ],fileName:_String|Automatic:Automatic,ops:OptionsPattern[]]:=
	...
	With[{
		...
		With[{file=OpenWrite[f],
			...
			~Join~
				...
				StringTemplate["AncestorClasses = ``"][parents]
				}
			~Join~
			attrs
			~Join~
...
(SaveObject[MathematicaClass[sym_?InstanceSymbolQ],fileName:_String|Automatic:Automatic,ops:OptionsPattern[]]):=
Block[{$SaveStack=Replace[$SaveStack,_Symbol:><||>]},
With[{dir=Replace[OptionValue@Directory,Except[_String]:>$ObjectDirectory]//If[OptionValue@CreateArchive//TrueQ,FileNameJoin@{#,ToString@sym},#]&,
		fullPaths=OptionValue@Path===Full},
	If[!DirectoryQ@dir,CreateDirectory@dir];
